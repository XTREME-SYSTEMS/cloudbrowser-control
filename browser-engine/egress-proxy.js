...
function normalizeUpstream(upstreamProxy) {
  ...
  if (!["http:", "https:"].includes(parsed.protocol)) {
    throw new EgressProxyError(400, `Unsupported upstream proxy protocol: ${parsed.protocol}`);
  }
  ...
  return {
    ...
    port,
    username: upstreamProxy.username || decodeURIComponent(parsed.username || ""),
    password: upstreamProxy.password || decodeURIComponent(parsed.password || ""),
  };
}
...
export async function resolvePinnedTarget(rawUrl, policy = {}, resolver = dns.lookup) {
  ...
  return {
    url: verdict.parsed,
    hostname: verdict.parsed.hostname,
    port: verdict.port,
    address,
...
async function resolvePinnedUpstream(upstream, resolver) {
  ...
  const target = await resolvePinnedTarget(`${scheme}://${bracketHost(upstream.hostname)}:${upstream.port}/`, {
...
function openSocket({ address, port, family, tlsServerName, tlsEnabled = false }) {
  return new Promise((resolve, reject) => {
    ...
    const socket = tlsEnabled
      ? tls.connect({ host: address, port, family, servername: tlsServerName, rejectUnauthorized: true }, () => {
          socket.off("error", onError);
          resolve(socket);
        })
      : net.connect({ host: address, port, family }, () => {
          socket.off("error", onError);
          resolve(socket);
        });
...
async function openUpstreamTunnel(target, upstream, resolver) {
  ...
  const socket = await openSocket({
    address: pinnedUpstream.address,
    port: pinnedUpstream.port,
    family: pinnedUpstream.family,
    tlsServerName: pinnedUpstream.hostname,
    tlsEnabled: pinnedUpstream.protocol === "https:",
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      if (marker < 0) {
        if (buffered.length > 65536) reject(new Error("Upstream proxy CONNECT response too large"));
        return;
      }
      ...
      if (status !== 200) {
        socket.destroy();
        reject(new Error(`Upstream proxy CONNECT failed with status ${status || "unknown"}`));
        return;
      }
...
function copyTargetHeaders(headers, host) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (upstream) {
        ...
        requestOptions = {
          hostname: pinnedUpstream.address,
          port: pinnedUpstream.port,
          family: pinnedUpstream.family,
          servername: pinnedUpstream.protocol === "https:" ? pinnedUpstream.hostname : undefined,
          method: req.method,
          path: `http://${bracketHost(target.address)}:${target.port}${requestPath}`,
          headers,
        };
      } else {
        requestOptions = {
      ...
      const outbound = transport.request(requestOptions, (upstreamRes) => {
      ...
      outbound.on("error", (error) => {
        ...
        res.end(JSON.stringify({ error: `Pinned egress request failed: ${error.message}` }));
      });
      req.pipe(outbound);
    } catch (error) {
      stats.denied++;
      const status = error instanceof EgressProxyError ? error.status : 502;
  ...
  server.on("connect", async (req, clientSocket, head) => {
    ...
    try {
      ...
      const targetSocket = upstream
        ? await openUpstreamTunnel(target, upstream, resolver)
        : await openSocket({ address: target.address, port: target.port, family: target.family });
      ...
      targetSocket.on("error", destroy);
    } catch (error) {
      stats.denied++;
      writeProxyError(clientSocket, error instanceof EgressProxyError ? error.status : 502, error.message);
    }
  });
