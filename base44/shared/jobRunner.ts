...
async function claimExecution(base44, job, idempotencyKey) {
  ...
  const existingSummary = job.results_summary && typeof job.results_summary === "object" ? job.results_summary : {};
  const claimSummary = {
    ...existingSummary,
    idempotency: {
  ...
  throw new JobRunnerError(409, "Job execution already claimed by another request", {
    job_id: job.id,
...
export async function executeJob(base44, options) {
  ...
  if (!await isEngineConfigured()) {
    throw new JobRunnerError(503, "Browser engine not configured");
  }
  ...
  if (steps.length > maxSteps) {
    await finishExecution(base44, jobId, fingerprint, "failed", {
      status: "failed",
      error_message: `Step count ${steps.length} exceeds max ${maxSteps}`,
      completed_at: new Date().toISOString(),
    });
  ...
  try {
    ...
    sessionEntity = await base44.asServiceRole.entities.Session.create({
      ...
      metadata: { job_id: jobId, project_id: job.project_id || null },
    });
    ...
    throw new JobRunnerError(500, `Session creation failed: ${err.message}`);
  }
  ...
  await finishExecution(base44, jobId, fingerprint, failed ? "failed" : "completed", {
  ...
  try { await calculateJobCost(base44, jobId); } catch (e) { console.error("Cost calculation failed:", e.message); }
  ...
  return {
    ...
    idempotency_state: failed ? "failed" : "completed",
    __v: DEPLOYMENT_VERSION,
  };
}
